	...
	EEex_HookBeforeRestoreWithLabels(0x1402B9EB2, 0, 0, 16, {
		...
		EEex_FlattenTable({
			...
			{[[
				...
				mov dword ptr [r14+0x768], 6

				mov rcx, ]]}, {EEex_WriteStringCache("CHARGEN_KIT")},
			{[[	
				#ALIGN
				call 0x1403DA620
				#ALIGN_END
	...
	EEex_HookBeforeCallWithLabels(0X1402B8CE1, {
		...
		EEex_FlattenTable({
			...
			{[[
				...
				cmp eax, 1
				je continue_kit_selection

				cmp eax, 2
				jne normal_continue

				mov dword ptr [r14+0x768], 0x19
...
function ST_RegisterHook_ProficiencyUIMax()
	EEex_HookBeforeRestoreWithLabels(0x1402C75C1, 0, 8, 8, {
		...
		EEex_FlattenTable({
			...
			EEex_GenLuaCall("ST_Hook_ProficiencyUIMax", {
				["args"] = {
...
function ST_Hook_ProficiencyUIMax(proficiencyId, proficiencyMax)
	local sprite = EEex_Sprite_GetSelected() or charGenSprite
	return ST_ProficiencyMax(sprite, proficiencyId, proficiencyMax)
end
...
function ST_RegisterHook_ProficiencyActualMax()
	EEex_HookBeforeRestoreWithLabels(0x1402BDC31, 0, 8, 8, {
		...
		EEex_FlattenTable({
			...
			EEex_GenLuaCall("ST_Hook_ProficiencyActualMax", {
				["args"] = {
					function(rspOffset) return {"mov qword ptr ss:[rsp+#$(1)], rsi #ENDL", {rspOffset}} end,
					function(rspOffset) return {"mov qword ptr ss:[rsp+#$(1)], rax #ENDL", {rspOffset}} end,
				},
...
function ST_Hook_ProficiencyActualMax(proficiencyId, proficiencyMax)
	local sprite = EEex_Sprite_GetSelected() or charGenSprite
	return ST_ProficiencyMax(sprite, proficiencyId, proficiencyMax)
end
